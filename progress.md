...
- 맵: 직선 낙하 구간을 줄이기 위해 지그재그 코리도 + 중간 넓은 챔버(프로펠러 회전 막대 2개로 섞기) 레이아웃 추가(`layout: "zigzag"`).
- Zigzag 개선: 코리도 centerline 키를 더 촘촘히/고정 좌우 교차로 조정해 직선 낙하 구간을 더 줄임.
- 믹싱 챔버: 프로펠러를 "단일 회전 막대"로 바꾸고(교차 막대 제거), 약한 하향 바이어스를 추가해 챔버에서 공이 영구적으로 맴도는 정체를 방지.
- 드레인 안정성: 챔버 이후 구간의 폭을 소폭 확장해(후반 deadlock 감소) 다수 Drop에서도 끝까지 내려가도록 튜닝.
- 테스트: 엔진에 `state.stats.propellerContacts` 추가(시뮬레이션 관측용) + `zigzag` 맵이 가로 이동/프로펠러 접촉이 발생하는지 단위 테스트 추가.
- 공정성: 시작 구간은 지그재그 대신 넓은 직선 통로로 변경(스폰 시 좌/우 강제 편향 제거). 이후부터 지그재그가 시작됨.
- 초반 섞기: 시작 구간(넓은 통로)에도 중앙 프로펠러(회전 막대) 1개를 추가해 초반에 한 번 섞이도록 개선. 다수 Drop에서도 공이 프로펠러 근처에 갇히지 않도록 짧은 길이/완만한 회전/패스 가능한 갭/상향 속도 제한으로 튜닝.
- 초반 프로펠러: 1개 긴 막대 대신 작은 프로펠러 여러 개로 변경(해당 영역 너비를 분할해 전폭을 커버). 회전 방향을 교차해 한쪽으로 쏠리는 편향을 줄임.
- 안정성(끼임 방지): 지그재그에서 좁은 포켓에 갇혀 왕복하는 케이스를 막기 위해, 일정 시간 창에서 하강 진행이 거의 없고 범위가 좁으면 결정론적 하향 nudge를 주는 방식으로 unstuck 로직 강화.
- 물리감 튜닝: 공의 움직임이 너무 빠르고 가벼워 보이던 문제를 완화하기 위해 `step()`의 중력/반발/감쇠/속도 상한을 조정해 더 무겁고 느린 느낌으로 변경.

## 2026-02-15
- UI polish: 공 설정 버튼 강조, 공 이름 hover title, 공 아이콘 텍스트 중앙/일관 정렬, 미니맵 힌트 줄바꿈, BGM 자동재생 정책 대응(제스처 후 시작), CodeRabbit 지적 반영(lang=ko, keyframes kebab-case, SVG 색상 sanitize, dead badge 코드 제거)
